...
--
-- Host: localhost    Database: assignment_5
-- ------------------------------------------------------
...
INSERT INTO `standings` VALUES (1,1,1,3,1,1,0,0,3,1),(2,1,2,0,1,0,0,1,1,3),(3,2,1,3,1,1,0,0,4,1),(4,2,2,0,1,0,0,1,2,2),(5,2,3,3,1,1,0,0,1,2),(6,2,4,3,1,1,0,0,2,1),(7,2,5,0,1,0,0,1,1,2),(8,2,6,1,1,0,1,0,2,2),(9,2,7,3,1,1,0,0,1,0),(10,3,1,3,1,1,0,0,1,2),(11,3,2,1,1,0,1,0,2,2),(12,3,3,0,1,0,0,1,1,2),(13,3,4,0,1,0,0,1,1,2),(14,3,5,0,1,0,0,1,1,2),(15,3,6,0,1,0,0,1,1,2),(16,3,7,0,1,0,0,1,1,2),(17,4,1,3,1,1,0,0,4,1),(18,4,2,0,1,0,0,1,3,4),(19,4,3,3,1,1,0,0,1,2),(20,4,4,3,1,1,0,0,2,1),(21,4,5,0,1,0,0,1,2,3),(22,4,6,1,1,0,1,0,1,1),(23,4,7,3,1,1,0,0,3,0);
...
--
-- Temporary view structure for view `tournament_number_1`
--

DROP TABLE IF EXISTS `tournament_number_1`;
/*!50001 DROP VIEW IF EXISTS `tournament_number_1`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `tournament_number_1` AS SELECT 
 1 AS `id`,
 1 AS `match_day_id`,
 1 AS `team_id`,
 1 AS `points`,
...
--
-- Temporary view structure for view `tournament_number_2`
--

DROP TABLE IF EXISTS `tournament_number_2`;
/*!50001 DROP VIEW IF EXISTS `tournament_number_2`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `tournament_number_2` AS SELECT 
 1 AS `id`,
 1 AS `match_day_id`,
 1 AS `team_id`,
 1 AS `points`,
...
--
-- Final view structure for view `tournament_number_1`
--
...
/*!50001 VIEW `tournament_number_1` AS select `s`.`id` AS `id`,`s`.`match_day_id` AS `match_day_id`,`s`.`team_id` AS `team_id`,`s`.`points` AS `points`,`s`.`played` AS `played`,`s`.`won` AS `won`,`s`.`drawn` AS `drawn`,`s`.`lost` AS `lost`,`s`.`goals_for` AS `goals_for`,`s`.`goals_against` AS `goals_against`,`l`.`name` AS `league_name`,`t`.`name` AS `team_name` from (((`standings` `s` join `match_days` `md` on((`s`.`match_day_id` = `md`.`id`))) join `leagues` `l` on((`md`.`league_id` = `l`.`id`))) join `teams` `t` on((`s`.`team_id` = `t`.`id`))) where (`s`.`match_day_id` = (select max(`s2`.`match_day_id`) from (`standings` `s2` join `match_days` `md2` on((`s2`.`match_day_id` = `md2`.`id`))) where (`md2`.`league_id` = 1))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `tournament_number_2`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `tournament_number_2` AS select `s`.`id` AS `id`,`s`.`match_day_id` AS `match_day_id`,`s`.`team_id` AS `team_id`,`s`.`points` AS `points`,`s`.`played` AS `played`,`s`.`won` AS `won`,`s`.`drawn` AS `drawn`,`s`.`lost` AS `lost`,`s`.`goals_for` AS `goals_for`,`s`.`goals_against` AS `goals_against`,`l`.`name` AS `league_name`,`t`.`name` AS `team_name` from (((`standings` `s` join `match_days` `md` on((`s`.`match_day_id` = `md`.`id`))) join `leagues` `l` on((`md`.`league_id` = `l`.`id`))) join `teams` `t` on((`s`.`team_id` = `t`.`id`))) where (`s`.`match_day_id` = (select max(`s2`.`match_day_id`) from (`standings` `s2` join `match_days` `md2` on((`s2`.`match_day_id` = `md2`.`id`))) where (`md2`.`league_id` = 2))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
